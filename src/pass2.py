...
def rename(src, mapping, lo=0, hi=None):
    ...
    brace, _d = bytearray(len(src)), 0
    for _i, _c in enumerate(src):
        if mask[_i]:
            if _c == '{': _d += 1
    ...
    for m in IDENT.finditer(src):
        ...
        # a table KEY is `name =` inside a table constructor. A multi-assignment
        # `local a, b, c =` sits at brace depth 0 and must still be renamed.
        if (brace[m.start()] > 0 and k < len(src) and src[k] == '=' and src[k:k+2] != '=='
                and (j < 0 or src[j] in '{,' or src[j] == chr(10))):
            continue
